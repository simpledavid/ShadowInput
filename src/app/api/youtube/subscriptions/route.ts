...
export async function GET(request: NextRequest) {
  ...
  const cached = await getCachedSubscriptions(db, userId);
  if (cached.length > 0) {
  ...
  if (new Date(tokenRow.expires_at).getTime() <= Date.now() + 5 * 60 * 1000) {
    ...
    await db
      ...
      .bind(accessToken, new Date(Date.now() + (refreshed.expires_in ?? 3600) * 1000).toISOString(), userId)
      .run();
  }
  ...
  await replaceSubscriptions(db, userId, channels);

  const now = new Date().toISOString();
  return NextResponse.json({
    channels: channels.map(c => ({ ...c, user_id: userId, fetched_at: now })),
    fromCache: false,
  });
}
